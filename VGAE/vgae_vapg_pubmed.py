...
from evaluate import cluster_metrics
from utils import normalize_adj, sparse_mx_to_torch_sparse_tensor, normalize_adj_v2
from vbpg import VBPG
np.random.seed(826)
...
def load_data(name):
    r = "H:\Jung\Studio\Studio\Jung\datasets\\"
...
if __name__ == "__main__":
    ...
    for epoch in range(100):
        ...
        if epoch % 20 == 0:
            if epoch == 0:
                update_dis = False
            mean_adj, delta_add, delta_dele = plug.cal_mean(delta_add, delta_dele, epsilon=3, sin_iter=2, eta=0.2,
            ...
            var_adj, delta_var_add, delta_var_dele = plug.cal_var(delta_var_add, delta_var_dele, epsilon=3, sin_iter=2,
                                                                  eta=0.8, k=len(np.unique(model.label)), pre_label=best_pred,
